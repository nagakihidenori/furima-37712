  ...
  describe '配送先情報の保存' do
    ...
    context '配送先情報の保存ができないとき' do
      ...
      it '番地が空だと保存できないこと' do
        @order_form.cities = nil
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include("Cities can't be blank")
      end
      ...
      it '電話番号にハイフンがあると保存できないこと' do
        @order_form.tell_num = '123 - 1234 - 1234'
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include('Tell num is invalid')
      end
      it '電話番号が12桁以上あると保存できないこと' do
        @order_form.tell_num = 12_345_678_910_123_111
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include('Tell num is invalid')
      end
      it '電話番号が9桁以下だと保存できないこと' do
        @order_form.tell_num = 123_456_789
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include('Tell num is invalid')
      end
      it '電話番号に半角数字以外が含まれている場合は購入できない' do
        @order_form.tell_num = "123_4567_890１"
        @order_form.valid?
        expect(@order_form.errors.full_messages).to include('Tell num is invalid')
      end
